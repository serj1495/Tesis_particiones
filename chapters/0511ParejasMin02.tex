...
%Falta mostrar que si una pareja $P$ en $S(G)$$ no está en $\mu(G)$, entonces $P$ domina a algún elemento de $\mu(G)$. Esto me queda claro porque si hubiera un elemento de $S(G)$ fuera de $\mu(G)$ que no dominara a ningún elemento de $\mu(G)$, entonces dominaría a otro elemento en $S(G)$ que tampoco dominaría a nadie en $\mu(G)$. Esto no puede continuar hasta el infinito porque no pueden haber dos elementos de $S(G)$ diferentes que se dominen mutuamente. Pero no sé cómo escribir esto.
